...
func TestShutdownHookConcurrency(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		go func(id int) {
			...
			manager.AddHook(ShutdownPhaseCleanup, hook)
			done <- struct{}{}
		}(i)
	}
...
func TestWorkerShutdownHooks(t *testing.T) {
	...
	// Pre-stop hook
	err := worker.Worker.AddPreStopHook("test_prestop", func(ctx context.Context) error {
	...
	// Cleanup hook
	err = worker.Worker.AddCleanupHook("test_cleanup", func(ctx context.Context) error {
		atomic.AddInt32(&hooksCalled, 1)
		return nil
	})
	assert.NoError(t, err)
	...
	// Custom phase hook
	err = worker.Worker.AddShutdownHook(ShutdownPhasePostJobs, ShutdownHook{
		Name: "test_postjobs",
		Handler: func(ctx context.Context) error {
			atomic.AddInt32(&hooksCalled, 1)
			return nil
	...
	preStopHooks := worker.Worker.GetShutdownHooks(ShutdownPhasePreStop)
	assert.Len(t, preStopHooks, 1)
	
	cleanupHooks := worker.Worker.GetShutdownHooks(ShutdownPhaseCleanup)
	assert.Len(t, cleanupHooks, 1)
	
	postJobsHooks := worker.Worker.GetShutdownHooks(ShutdownPhasePostJobs)
	assert.Len(t, postJobsHooks, 1)
	
	// Remove a hook
	removed := worker.Worker.RemoveShutdownHook(ShutdownPhaseCleanup, "test_cleanup")
	assert.True(t, removed)
