...
//
// This function is specifically designed for real-time S3 uploads, where each
// segment needs to be normalized independently before upload. Unlike
// normalizeHLSTimestamps (which normalizes all segments using a shared reference),
// this function analyzes and normalizes each segment self-contained.
//
// Algorithm:
//  1. Analyze the segment file to find the first PTS and PCR values
//  2. Use these values as offsets for normalization
//  3. Rewrite the segment file with normalized timestamps
//
// Use cases:
//   - Real-time S3 uploader: Normalize segments as they are created
//   - Single segment processing: When you need to normalize one file independently
//
// The normalization ensures:
//   - Timestamps start near zero within each segment
//   - Playback compatibility with video players expecting low timestamp values
//   - Correct wraparound handling for long recordings
//
// Parameters:
//   - segmentPath: Absolute path to the MPEG-TS segment file
//
// Returns:
//   - nil if normalization succeeds or offsets are already zero
//   - error if segment cannot be read, analyzed, or normalized
//
// Note: If used in a multi-segment recording, each segment will have timestamps
// starting near zero independently. For batch normalization of all segments with
// a shared reference, use normalizeHLSTimestamps instead.
